...
import type { parseCrmRoute } from "./crm-routes";
...
export function useCrmUserAdminHandlers({
  ...
  const currentAdminId = useMemo(
    () => crmSession?.id ?? getCrmSessionFromToken()?.id ?? null,
    [crmSession]
  );
  ...
  const handleSavePlatformAdmin = async () => {
    ...
    if (route.userId === "new") {
      ...
      await load();
      if (res.admin?.id) {
        navigate(`/crm/users/${res.admin.id}`);
      } else {
        navigate("/crm/users");
      }
      return;
  ...
  const handleDeletePlatformAdmin = async () => {
    ...
    const session = getCrmSessionFromToken();
    if (session?.id === selectedUserId) {
